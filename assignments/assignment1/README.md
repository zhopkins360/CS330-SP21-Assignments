...
Through this assignment, you will gain hands-on experience with real-world network
...
**macOS only:** If you have installed Xcode, it comes with a binary package of 
Git and you don't have to install it. You may also install Git through Homebrew.

Once you have opened the .dmg installation file, you will see a
...
Run `git clone https://github.com/CSIS-BU/CS330-SP21-Assignments` to
download the course files from GitHub.

`cd CS330-SP21-Assignments/assignments` to enter the course assignment directory.
...
as "default: stdin: is not a tty", but you shouldn't have to worry about them.
...
to `vagrant@cs330:~$`. All further commands will execute on the VM. You can
...
You can choose either language (only one). The Python solution is shorter.
...
* Each client program should contact a server, read a message from stdin (standard
  input, i.e., keyboard), send the message, and exit.
...
To get yourself started, you may refer to [Socket Programming HOWTO](https://docs.python.org/3/howto/sockets.html).
...
top, the [section on socket objects](https://docs.python.org/3/library/socket.html#socket-objects) and the
...
5. Several long, random alphaumeric messages sent concurrently from separate
...
on Canvas or see the instructor during office hours.
...
* **I'm getting an error when I run the command `vagrant up`. What do I do?**
  ...
  * **"Vagrant cannot forward the specified ports on this VM, since they
  ...
  If this did not help you fix the problem, please ask on Canvas or at office
  hours.
...
[COS 461, Princeton University](https://www.cs.princeton.edu/courses/archive/spring19/cos461/)
